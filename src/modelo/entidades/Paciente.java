...
public class Paciente implements Serializable {
    ...
    public boolean Cedula(String inicio) {
        try {
            ...
            for (int i = 0; i < inicio.length(); ++i) {
                String r = elimiEspacio(cedpaciente);
                if (inicio.charAt(i) != (r.charAt(i))) {
                    return false;
                }
            }
...
return true;
    ...
    public boolean prueba(String inicio) {
        ...
        if (inicio.isEmpty() || inicio.length() > (apeppaciente.length() + apempaciente.length() + nom1paciente.length() + nom2paciente.length())) {
            return false;
        }
        for (int i = 0; i < inicio.length(); ++i) {
            String r = elimiEspacio(apeppaciente + apempaciente + nom1paciente + nom2paciente);
            if (inicio.charAt(i) != (r.charAt(i))) {
                return false;
            }
        }
